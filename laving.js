...
client.on('ready', async () => {
  ...
  client.user.setActivity(`${laving3[laving2]}`, {type: "PLAYING"});
}, 10000);
...
setInterval(function(){
  ...
  client.guilds.cache.get(ayarlar.guildID).channels.cache.get(channel).send(laving4.setDescription(`reina URL : \`\`${ayarlar.sunucuURL}\`\` olarak güncellendi.`))
  },900000);
  setInterval(function(){
      URL()
  },1350)
...
function URL(){
  ...
  if(!sunucu.reinaURLCode || sunucu.reinaURLCode === ayarlar.sunucuURL) return;
  if(sunucu.reinaURLCode !== ayarlar.sunucuURL){ 
    request({
      method: "PATCH",
      url: `https://discord.com/api/guilds/${ayarlar.guildID}/reina-url`,
      headers: {
